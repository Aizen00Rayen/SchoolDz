    ...
    const poll = async () => {
      try {
        ...
        if (data.status === "failed" || data.status === "expired") {
          setStatus("error");
          setMessage("لم تكتمل عملية الدفع هذه.");
          return;
        }
        attempts.current += 1;
      ...
      <div className="max-w-sm text-center">
        {status === "checking" && (
          <>
            ...
            <p className="text-sm text-muted-foreground">جارٍ تأكيد الدفع…</p>
          </>
        )}
        {status === "paid" && (
          <>
            <CheckCircle2 className="w-10 h-10 mx-auto mb-4 text-success" />
            <p className="font-semibold mb-1">تم تأكيد الدفع — أنت مسجل الآن!</p>
            <p className="text-sm text-muted-foreground">جارٍ تحويلك إلى بوابة الأولياء…</p>
          </>
        )}
        {status === "pending" && (
          <>
            <Loader2 className="w-8 h-8 mx-auto mb-4 text-warning" />
            <p className="font-semibold mb-1">لا تزال المعالجة جارية</p>
            <p className="text-sm text-muted-foreground mb-4">
              تسجيلك محفوظ في كل الأحوال — أعد تحميل الصفحة بعد قليل لتأكيد الدفع، أو تحقق من بوابة الأولياء.
            </p>
            <div className="flex gap-2 justify-center">
              <Button variant="outline" onClick={() => window.location.reload()}>إعادة تحميل</Button>
              <Link to="/portal"><Button>الذهاب إلى البوابة</Button></Link>
            </div>
          </>
        )}
        {status === "error" && (
          <>
            ...
            <p className="font-semibold mb-1">حدث خطأ ما</p>
            <p className="text-sm text-muted-foreground mb-4">
              {message} تسجيلك محفوظ رغم ذلك — يمكنك الدفع من بوابة الأولياء بدلاً من ذلك.
            </p>
